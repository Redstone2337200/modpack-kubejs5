                    ...
                    .then(Commands.argument('lvl', Arguments.INTEGER.create(event))
                        .then(Commands.argument('bool', Arguments.BOOLEAN.create(event))
                            .executes(run => {
                                ...
                                let tags = ["defvip", "opvip", "vip", "defsvip", "svip"]
                                ...
                                for (const type of types) {
                                    tags.forEach(tag => {
                                        ...
                                        } else if (target != null || target != undefined && !player.stages.has('opsvip') && !player.stages.has(tag) && on == types[0] && lvl <= 1 && admin && admin && bool == true) {
                                            event.server.tell(Text.green(`执行成功!恭喜玩家${player}成功晋升至管理超会。`).bold())
                                            player.stages.add('opsvip')
                                        } else if (target != null || target != undefined && player.stages.has('opsvip') && !player.stages.has(tag) && on == types[1] && lvl <= 1 && admin && bool == true) {
                                            event.server.tell(Text.green(`执行成功！已将管理超会玩家${player}移出了管理超会行列。`).bold())
                                            player.stages.remove('opsvip')
                                        }
                                    })
                                }
                            }
...
onEvent('player.chat', function (event) {
    ...
    let id = event.player;
    if (event.player.stages.has('opsvip') && event.player.op) {
